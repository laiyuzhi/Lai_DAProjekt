...
class InputBlock(nn.Module):
    def __init__(self, input, output, dropout=0):
        ...
        self.fc1 = nn.Linear(input, int(output/4))
        nn.init.kaiming_uniform_(self.fc1.weight)
        self.bn1 = nn.BatchNorm1d(int(output/4))
        self.relu1 = nn.ReLU()
        self.dropout1 = nn.Dropout(dropout)
        self.fc3 = nn.Linear(int(output/4), int(output/2))
        nn.init.kaiming_uniform_(self.fc3.weight)
        self.bn2 = nn.BatchNorm1d(int(output/2))
        self.relu3 = nn.ReLU()
        self.dropout2 = nn.Dropout(dropout)
        self.fc4 = nn.Linear(int(output/2), output)
        nn.init.kaiming_uniform_(self.fc4.weight)
        self.bn3 = nn.BatchNorm1d(int(output))
        self.relu4 = nn.ReLU()
...
class MlpBlock(nn.Module):
    def __init__(self, input, output, dropout=0.0):
...
class MLP_Group(nn.Module):
    def __init__(self, block, input, output, n, dropoutRate=0.0):
        super(MLP_Group, self).__init__()
        self.layer = self.make_layer(block, input, output, n, dropoutRate)
    def make_layer(self, block, input, output, n, dropoutRate=0.0):
        layers = []
        for i in range(int(n)):
            layers.append(block(i==0 and input or output, output, dropoutRate))
...
class WideResNet(nn.Module):
    def __init__(self, depth, num_classes, widen_factor, res_factor=10, dropRate=0.0, dropRateDense=0.1, Fcn_depth=1):
        super(WideResNet, self).__init__()
        nChannels = [16, 16*widen_factor, 32*widen_factor, 64*widen_factor]
        fcnChannels = [2*res_factor, 3*res_factor, 4*res_factor]
        ...
        self.inputLayer = InputBlock(cfg.INPUT_MULTI, fcnChannels[2], dropRateDense)
        # fcn resblock2
        self.fcnblock2 = MLP_Group(MLPblock, fcnChannels[2] + fcnChannels[2], fcnChannels[1], Fcn_depth, dropRateDense)
        # fcv resblock3
        self.fcnblock3 = MLP_Group(MLPblock, fcnChannels[2] + fcnChannels[1] + fcnChannels[2], fcnChannels[1], Fcn_depth, dropRateDense)
